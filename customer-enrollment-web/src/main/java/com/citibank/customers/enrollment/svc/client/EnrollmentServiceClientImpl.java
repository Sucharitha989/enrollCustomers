...
Right click your eclipse project Properties -> Deployment Assembly -> Add -> Java Build Path Entries -> Gradle Dependencies -> Finish*/
		/*SEVERE: Servlet [Jersey-servelet] in web application [/enrollment-servicewar] threw load() exception
	...
	public static void main(String[] args) throws IOException {
		...
		clientContext.setChannelId("mobile");
		clientContext.setRequestId(UUID.randomUUID().toString());
		request1.setClientContext(clientContext);
		
		
		// 3.prepare the customer details
		...
		request1.setCustomerInfo(customerInfo);
		...
		EnrollmentServiceClientImpl svcClient = new EnrollmentServiceClientImpl();

		EnrollmentServiceRes resp = svcClient.enroll(request1);
